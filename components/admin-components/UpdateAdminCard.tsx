...
const UpdateAdminCard = () => {
  ...
  return (
    <div className="fixed pb-4 flex flex-col left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 bg-gray-50 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] rounded-lg overflow-x-hidden max-h-screen">
      <button
        type="button"
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-center">
            <Button
              ...
              variant={'ghost'}
              className="w-1/2 mt-3 text-muted-foreground text-xs text-right border-gray-300"
            >
              Proceed to edit{' '}
              {!openForm ? (
          ...
          {openForm && <EditAdminForm adminId={adminDetails._id} />}
        </div>
      </div>
    </div>
  );
